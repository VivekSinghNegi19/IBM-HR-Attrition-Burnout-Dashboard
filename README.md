...
BurnoutRiskScore =
    (5 - HR[WorkLifeBalance])
    + (5 - HR[JobSatisfaction])
    + (5 - HR[EnvironmentSatisfaction])
    + HR[YearsSinceLastPromotion]
    + IF(HR[OverTime] = "Yes", 3, 0)
    + IF(HR[BusinessTravel] = "Travel_Frequently", 2,
        IF(HR[BusinessTravel] = "Travel_Rarely", 1, 0))
Burnout Risk Categorization
BurnoutCategory =
    IF([BurnoutRiskScore] >= 15, "Critical",
    IF([BurnoutRiskScore] >= 10, "High",
    IF([BurnoutRiskScore] >= 6,  "Medium", "Low")))
Key Measures Built
-- Overall Attrition Rate
Attrition Rate =
    DIVIDE(SUM(HR[AttritionFlag]), COUNTROWS(HR), 0) * 100
